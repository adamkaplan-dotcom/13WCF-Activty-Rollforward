        ...
        if "Rollforward Check" in wb_target.sheetnames:
            ...
            if cumulative_col_original:
                ...
                if max_value_col_original:
                    ...
                    # Copy column widths for the 5 columns
                    for col_offset_loop in range(5):
                        source_col_letter = openpyxl.utils.get_column_letter(source_start_col + col_offset_loop)
                        target_col_letter = openpyxl.utils.get_column_letter(target_start_col + col_offset_loop)
                    ...
                    # Copy values/formulas only (skip formatting for speed)
                    cells_copied = 0
                    formulas_copied = 0
                    formulas_adjusted = 0

                    # Cap at reasonable row count for performance
                    ...
                    log.append(f"✓ Copied {cells_copied} cells ({formulas_copied} formulas, {formulas_adjusted} adjusted)")
                    ...
                    if mq_col <= sheet_rollforward_check.max_column and mw_col <= sheet_rollforward_check.max_column:
                        log.append(f"Copying formulas from MQ → MW...")
