...
def process_files(weekly_file_path, rollforward_file_path, bth_file_path=None):
    ...
    try:
        ...
        if "Rollforward Check" in wb_target.sheetnames:
            ...
            # STEP 11A: Find column with "cumulative" in row 10
            cumulative_col = None
            ...
            if cumulative_col:
                # STEP 11B: Insert 5 columns to the left of the cumulative column
                log.append(f"Inserting 5 columns before column {openpyxl.utils.get_column_letter(cumulative_col)}...")
                sheet_rollforward_check.insert_cols(cumulative_col, 5)
                log.append(f"✓ Inserted 5 columns. 'Cumulative' is now in column {openpyxl.utils.get_column_letter(cumulative_col + 5)}")

                # STEP 11C: Find the greatest value in row 8
                ...
                log.append("Scanning row 8 for greatest value...")
                ...
                if max_value_col:
                    log.append(f"✓ Greatest value in row 8: {max_value} in column {openpyxl.utils.get_column_letter(max_value_col)}")

                    # STEP 11D: Copy 5 columns (max column + 4 to the right) to the newly inserted columns
                    source_start_col = max_value_col
                    source_end_col = max_value_col + 4  # 5 columns total
                    target_start_col = cumulative_col  # The first of the 5 newly inserted columns

                    log.append(f"Copying columns {openpyxl.utils.get_column_letter(source_start_col)}-{openpyxl.utils.get_column_letter(source_end_col)} → {openpyxl.utils.get_column_letter(target_start_col)}-{openpyxl.utils.get_column_letter(target_start_col + 4)}")
                    ...
                    for row in range(1, sheet_rollforward_check.max_row + 1):
                        for col_offset in range(5):  # 5 columns
                            ...
                            # Copy formula or value
                            if source_cell.value and isinstance(source_cell.value, str) and source_cell.value.startswith('='):
                                # Copy formula without adjustment (exact copy)
                                target_cell.value = source_cell.value
                                cells_copied += 1
                            elif source_cell.value is not None:
                                target_cell.value = source_cell.value
                    ...
                    log.append(f"✓ Copied {cells_copied} cells to Rollforward Check inserted columns")
                else:
        ...
        if "Rollforward Check" in wb_target.sheetnames and 'cumulative_col' in locals() and cumulative_col:
            log.append(f"✓ Rollforward Check: Inserted 5 columns before 'Cumulative'")
