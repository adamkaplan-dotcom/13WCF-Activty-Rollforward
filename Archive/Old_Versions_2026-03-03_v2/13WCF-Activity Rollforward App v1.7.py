...
def process_files(weekly_file_path, rollforward_file_path, bth_file_path=None):
    ...
    try:
        ...
        if "Cockpit" in wb_target.sheetnames:
            ...
            # To find the MOST RECENT DATE in row 10, we need to load the file with data_only=True
            # (wb_target has formulas, not calculated values)
            log.append("Loading Cockpit tab with calculated values to find most recent date...")
            wb_temp = openpyxl.load_workbook(rollforward_file_path, data_only=True)
            sheet_cockpit_temp = wb_temp["Cockpit"]
            ...
            log.append(f"Max column in Cockpit: {sheet_cockpit_temp.max_column}")

            for col in range(1, sheet_cockpit_temp.max_column + 1):
                cell_val = sheet_cockpit_temp.cell(10, col).value

                # Debug logging for first 10 and last 10 columns
                if col <= 10 or col >= sheet_cockpit_temp.max_column - 10:
                    col_letter = openpyxl.utils.get_column_letter(col)
                    log.append(f"  {col_letter}10: {repr(cell_val)[:50]} (type: {type(cell_val).__name__})")
                ...
                if cell_val and isinstance(cell_val, datetime):
                    if most_recent_date is None or cell_val > most_recent_date:
                        ...
                        log.append(f"    → Found date: {cell_val} in column {openpyxl.utils.get_column_letter(col)}")
            ...
            if most_recent_date_col:
                ...
                log.append(f"Source column: {openpyxl.utils.get_column_letter(cockpit_prev_col)}")
                log.append(f"Target column: {openpyxl.utils.get_column_letter(cockpit_new_col)}")
                log.append(f"Cockpit max row: {sheet_cockpit.max_row}")
                ...
                for row in range(1, sheet_cockpit.max_row + 1):
                    ...
                    # Log details for important rows (10, 20, and first 5 rows)
                    should_log = row in [10, 20] or row <= 5
                    ...
                    if source_cell.value and isinstance(source_cell.value, str) and source_cell.value.startswith('='):
                        ...
                        if should_log:
                            log.append(f"  Row {row}: Formula copied")
                            log.append(f"    Source: {source_cell.value[:60]}")
                            log.append(f"    Target: {adjusted_formula[:60]}")
                    elif source_cell.value is not None:
                        target_cell.value = source_cell.value
                        cockpit_values_copied += 1

                        if should_log:
                            log.append(f"  Row {row}: Value copied: {source_cell.value}")
                ...
                if bth_file_path:
                    ...
                    try:
                        ...
                        if "Summary" in wb_bth.sheetnames:
                            ...
                            log.append(f"BTH max column: {sheet_bth.max_column}")
                            ...
                            for col in range(1, sheet_bth.max_column + 1):
                                ...
                                # Log first and last 10 columns
                                if col <= 10 or col >= sheet_bth.max_column - 10:
                                    col_letter = openpyxl.utils.get_column_letter(col)
                                    log.append(f"  BTH {col_letter}30: {repr(bth_date)[:50]}")
        ...
        if "Rollforward Check" in wb_target.sheetnames:
            ...
            if cumulative_col_original:
                # STEP 11B: Find the greatest value in row 8 (BEFORE any insertions)
                log.append("Loading Rollforward Check with calculated values to find greatest value in row 8...")
                wb_temp = openpyxl.load_workbook(rollforward_file_path, data_only=True)
                sheet_rollforward_temp = wb_temp["Rollforward Check"]

                max_value = None
                max_value_col_original = None

                log.append("Scanning row 8 for greatest value (before column insertion)...")
                for col in range(1, sheet_rollforward_temp.max_column + 1):
                    ...
                    if isinstance(cell_val, (int, float)):
                        if max_value is None or cell_val > max_value:
                            ...
                            col_letter = openpyxl.utils.get_column_letter(col)
                            log.append(f"  → New max: {cell_val} in column {col_letter}")

                wb_temp.close()

                if max_value_col_original:
                    log.append(f"✓ Greatest value in row 8: {max_value} in column {openpyxl.utils.get_column_letter(max_value_col_original)} (original position)")

                    # STEP 11C: Insert 6 columns (1 blank + 5 for data)
                    log.append(f"Inserting 6 columns before column {openpyxl.utils.get_column_letter(cumulative_col_original)}...")
                    sheet_rollforward_check.insert_cols(cumulative_col_original, 6)
                    log.append(f"✓ Inserted 6 columns. 'Cumulative' is now in column {openpyxl.utils.get_column_letter(cumulative_col_original + 6)}")

                    # STEP 11D: Adjust source column positions if they were affected by insertion
                    # If the max value column was at or after the insertion point, add 6 to its position
                    if max_value_col_original >= cumulative_col_original:
                        max_value_col_adjusted = max_value_col_original + 6
                        log.append(f"Source column shifted by insertion: {openpyxl.utils.get_column_letter(max_value_col_original)} → {openpyxl.utils.get_column_letter(max_value_col_adjusted)}")
                    else:
                        max_value_col_adjusted = max_value_col_original
                        log.append(f"Source column unaffected by insertion (before insertion point): {openpyxl.utils.get_column_letter(max_value_col_adjusted)}")

                    # STEP 11E: Copy 5 columns starting from max value column
                    # Target is the SECOND of the 6 inserted columns (skip first blank column)
                    source_start_col = max_value_col_adjusted
                    source_end_col = max_value_col_adjusted + 4  # 5 columns total
                    target_start_col = cumulative_col_original + 1  # Skip first blank column
                    target_end_col = cumulative_col_original + 5  # 5 columns of data

                    # Calculate column offset for formula adjustment
                    # We're copying from source columns to target columns, which are earlier in the sheet
                    column_offset = target_start_col - source_start_col

                    log.append(f"Copying columns {openpyxl.utils.get_column_letter(source_start_col)}-{openpyxl.utils.get_column_letter(source_end_col)} → {openpyxl.utils.get_column_letter(target_start_col)}-{openpyxl.utils.get_column_letter(target_end_col)}")
                    log.append(f"Column offset for formula adjustment: {column_offset}")
                    log.append(f"(First inserted column {openpyxl.utils.get_column_letter(cumulative_col_original)} left blank)")

                    # Copy all rows from source columns to target columns
                    from copy import copy
                    cells_copied = 0
                    formulas_copied = 0
                    formulas_adjusted = 0

                    for row in range(1, sheet_rollforward_check.max_row + 1):
                        for col_offset_loop in range(5):  # 5 columns
                            source_col = source_start_col + col_offset_loop
                            target_col = target_start_col + col_offset_loop
                            ...
                            # Copy value or formula (with adjustment)
                            if source_cell.value is not None:
                                if isinstance(source_cell.value, str) and source_cell.value.startswith('='):
                                    # Adjust formula column references
                    ...
                    log.append(f"✓ Copied {cells_copied} cells ({formulas_copied} formulas, {formulas_adjusted} adjusted) to Rollforward Check")

                    # STEP 11F: Copy formulas from MQ to MW (with adjustments)
                    mq_col = openpyxl.utils.column_index_from_string('MQ')
                    mw_col = openpyxl.utils.column_index_from_string('MW')

                    # Check if MQ and MW exist in the sheet
                    if mq_col <= sheet_rollforward_check.max_column and mw_col <= sheet_rollforward_check.max_column:
                        log.append(f"Copying formulas from MQ → MW with adjustments...")
                        mq_to_mw_offset = mw_col - mq_col
                        mq_formulas_copied = 0
                        ...
                        for row in range(1, sheet_rollforward_check.max_row + 1):
                            ...
                            if source_cell.value and isinstance(source_cell.value, str) and source_cell.value.startswith('='):
                                adjusted_formula = adjust_formula_columns(source_cell.value, column_offset=mq_to_mw_offset)
                                target_cell.value = adjusted_formula
                                mq_formulas_copied += 1
                            elif source_cell.value is not None:
                                target_cell.value = source_cell.value

                        log.append(f"✓ Copied {mq_formulas_copied} formulas from MQ → MW")
                    else:
                        log.append(f"⚠ Columns MQ or MW not found in sheet (max column: {openpyxl.utils.get_column_letter(sheet_rollforward_check.max_column)})")

                    # STEP 11G: Set column widths for MT through MX (16.67)
                    # MT = column 520, MX = column 524
                    mt_col = openpyxl.utils.column_index_from_string('MT')
                    mx_col = openpyxl.utils.column_index_from_string('MX')

                    log.append(f"Setting column widths for MT-MX to 16.67...")
                    for col in range(mt_col, mx_col + 1):
                        col_letter = openpyxl.utils.get_column_letter(col)
                        sheet_rollforward_check.column_dimensions[col_letter].width = 16.67

                    log.append(f"✓ Set width 16.67 for columns MT through MX")
        ...
        if "Rollforward Check" in wb_target.sheetnames and 'cumulative_col_original' in locals() and cumulative_col_original:
            log.append(f"✓ Rollforward Check: Inserted 6 columns (1 blank + 5 data)")
            if 'cells_copied' in locals():
                log.append(f"✓ Rollforward Check: Copied {cells_copied} cells with adjusted formulas")
            if 'mq_formulas_copied' in locals():
                log.append(f"✓ Rollforward Check: Copied {mq_formulas_copied} formulas from MQ → MW")
