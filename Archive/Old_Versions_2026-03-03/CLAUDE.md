...
python3 13WCF- Balance Updater App.py
```
...
```
BalanceUpdater/
├── 13WCF- Balance Updater App.py  # Main Flask application
...
Located in `13WCF- Balance Updater App.py`, lines ~25-223:
...
### Upload Constraints
- Max file size: 100MB (configurable in `13WCF- Balance Updater App.py` line 16)
...
Located in `13WCF- Balance Updater App.py`:
...
All hardcoded references are in `process_files()` function (13WCF- Balance Updater App.py):
...
### Flask Debug Mode
Debug mode is enabled by default (13WCF- Balance Updater App.py line ~316):
```python
