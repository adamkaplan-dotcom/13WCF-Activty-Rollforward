...
def process_files(weekly_file_path, rollforward_file_path, bth_file_path=None):
    ...
    try:
        ...
        if "Cockpit" in wb_target.sheetnames:
            ...
            # Find the column with the most recent date in Cockpit tab
            # Assuming dates are in row 1 (adjust if different)
            last_date_col_cockpit = None
            most_recent_date = None

            # Search for dates in the Cockpit tab (check multiple rows for date headers)
            for date_row in [1, 2, 3, 4]:  # Check first few rows for dates
                for col in range(1, sheet_cockpit.max_column + 1):
                    cell_val = sheet_cockpit.cell(date_row, col).value
