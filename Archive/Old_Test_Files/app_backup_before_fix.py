...
def adjust_formula_columns(formula, column_offset=1):
    ...
    def adjust_cell_ref(match):
        ...
        prefix_char = match.group(1) or ''  # Character before (e.g., '(', ',', '=')
        dollar1 = match.group(2) or ''      # Optional $ before first column
        col1_letters = match.group(3)       # First column letters
        rest = match.group(4) or ''         # Everything after column

        # Only adjust if column reference is relative (no $ before column)
        if dollar1:
            # Absolute column reference - don't adjust
            new_col1 = col1_letters
        else:
            # Relative column reference - adjust it
            col1_num = col_to_num(col1_letters)
            new_col1 = num_to_col(col1_num + column_offset)

        # Check if there's a second column in a range (e.g., :$B or :B5)
        if rest.startswith(':'):
            # Extract second column from range
            range_match = re.match(r':(\$?)([A-Z]+)(.*)$', rest)
            if range_match:
                dollar2 = range_match.group(1) or ''
                col2_letters = range_match.group(2)
                trailing = range_match.group(3) or ''

                # Only adjust second column if relative (no $)
                if dollar2:
                    # Absolute - don't adjust
                    new_col2 = col2_letters
                else:
                    # Relative - adjust it
                    col2_num = col_to_num(col2_letters)
                    new_col2 = num_to_col(col2_num + column_offset)

                return prefix_char + dollar1 + new_col1 + ':' + dollar2 + new_col2 + trailing

        # No second column, just return adjusted first column
        return prefix_char + dollar1 + new_col1 + rest

    # Regex pattern to match cell/column references:
    # Must be preceded by a non-letter character (avoid matching function names)
    # Captures: (prefix)($ )(COL)(rest including row, $, range, etc.)
    pattern = r'(^|[^A-Za-z])(\$?)([A-Z]+)(\$?\d+|:\$?[A-Z]+\$?\d*|:\$?[A-Z]+|(?=[^A-Za-z0-9]|$))'
